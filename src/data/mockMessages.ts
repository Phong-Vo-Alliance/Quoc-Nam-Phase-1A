import { Message } from "@/features/portal/types";

import img1 from "@/assets/message/img/img1.jpg";
import img2 from "@/assets/message/img/img2.jpg";
import img3 from "@/assets/message/img/img3.jpg";
import img4 from "@/assets/message/img/img4.jpg";
import img5 from "@/assets/message/img/img5.jpg";
import img6 from "@/assets/message/img/img6.jpg";
import img7 from "@/assets/message/img/img7.jpg";
import img8 from "@/assets/message/img/img8.jpg";

// Helper tạo ISO + time dạng "HH:mm"
const makeTime = (h: number, m: number) => ({
  createdAt: new Date(2025, 10, 12, h, m).toISOString(),
  time: `${h.toString().padStart(2, "0")}:${m.toString().padStart(2, "0")}`,
});

/** 
 * CHÚ THÍCH:
 * - groupId: "grp_vh_kho" | "grp_vh_taixe"
 * - senderId + sender: thống nhất giữa userId và tên hiển thị
 * - AttachmentType chỉ có "image" | "pdf" | "excel"
 */

export const nhanHangMessages: Message[] = [
  {
    id: "sys_nh_0001",
    groupId: "grp_vh_kho",
    senderId: "system",
    sender: "System",
    type: "system",
    content: "Wednesday, Nov 12",
    ...makeTime(8, 0),
  },
  {
    id: "nh_1001",
    groupId: "grp_vh_kho",
    senderId: "u_thu_an",
    sender: "Thu An",
    type: "text",
    content: "Hàng đợt 2 đã về kho lúc 9h30, em bắt đầu kiểm đếm.",
    ...makeTime(9, 35),
  },
  {
    id: "nh_1002",
    groupId: "grp_vh_kho",
    senderId: "u_thanh_truc",
    sender: "Thanh Trúc",
    type: "text",
    content: "Nhớ lập biên bản nhận hàng nhé.",
    ...makeTime(9, 40),
    replyTo: {
      id: "nh_1001",
      type: "text",
      sender: "Thu An",
      content: "Hàng đợt 2 đã về kho lúc 9h30, em bắt đầu kiểm đếm.",
    },
  },
  {
    id: "nh_1003",
    groupId: "grp_vh_kho",
    senderId: "u_thu_an",
    sender: "Thu An",
    type: "image",
    content: "Phiếu nhập kho đợt 2",
    files: [{ name: "img1.jpg", url: img1, type: "image" }],
    ...makeTime(9, 45),
  },
  {
    id: "nh_1004",
    groupId: "grp_vh_kho",
    senderId: "u_diem_chi",
    sender: "Diễm Chi",
    type: "image",
    isMine: true,
    content: "Sắp xếp hàng trên kệ",
    files: [
      { name: "img2.jpg", url: img2, type: "image" },      
    ],
    ...makeTime(9, 55),
  },
  {
    id: "nh_10041",
    groupId: "grp_vh_kho",
    senderId: "u_thu_an",
    sender: "Thu An",
    type: "image",
    isMine: false,
    content: "Sắp xếp hàng trên kệ",
    files: [      
      { name: "img3.jpg", url: img3, type: "image" },
      { name: "img4.jpg", url: img4, type: "image" },
    ],
    ...makeTime(9, 55),
  },
  {
    id: "nh_1005",
    groupId: "grp_vh_kho",
    senderId: "u_thanh_truc",
    sender: "Thanh Trúc",
    type: "file",
    content: "Biên bản nhập kho đợt 2",
    fileInfo: { name: "bien_ban_nhap.pdf", url: "/files/bien_ban_nhap.pdf", type: "pdf", size: "256KB" },
    ...makeTime(10, 5),
  },
  {
    id: "msg_0001",
    groupId: "grp_vh_kho",
    senderId: "u_thanh_truc",
    sender: "Thanh Trúc",
    type: "text",
    content: "Kiểm tra biên bản nhận hàng đợt 2",    
    ...makeTime(12, 12),
    taskId: "task_001",
  },
  {
    id: "msg_0020",
    groupId: "grp_vh_kho",
    senderId: "u_diem_chi",
    sender: "Diễm Chi",    
    type: "text",
    isMine: true,
    content: "Theo dõi phiếu đổi trả NCC tuần 45",
    ...makeTime(11, 35),
    taskId: "task_005",
  },
  {
    id: "msg_0030",
    groupId: "grp_vh_kho",
    senderId: "u_thanh_truc",
    sender: "Thanh Trúc",
    type: "text",
    content: "Phê duyệt kế hoạch nhập hàng tháng 12",
    ...makeTime(13, 8),
  },
  {
    id: "msg_0040",
    groupId: "grp_vh_kho",
    senderId: "u_le_binh",
    sender: "Le Bình",
    type: "image",
    content: "Chuẩn bị nhận hàng mới",
    files: [      
      { name: "img5.jpg", url: img5, type: "image" },      
    ],
    ...makeTime(13, 40),
  },
  {
    id: "msg_0050",
    groupId: "grp_vh_kho",
    senderId: "u_le_binh",
    sender: "Lệ Bình",
    type: "text",
    content: "Bắt đầu nhận hàng mới từ NCC.",
    ...makeTime(13, 8),
  },
  {
    id: "msg_0060",
    groupId: "grp_vh_kho",
    senderId: "u_diem_chi",
    sender: "Diễm Chi",
    type: "text",
    content: "Hàng đang được xuống từ xe vào kho.",
    ...makeTime(13, 30),
  },
  {
    id: "msg_0070",
    groupId: "grp_vh_kho",
    senderId: "u_diem_chi",
    sender: "Diễm Chi",
    type: "image",
    isMine: true,
    content: "Xuống hàng vào kho",
    files: [      
      { name: "img6.jpg", url: img6, type: "image" },      
    ],
    ...makeTime(13, 40),
  },
  {
    id: "msg_0080",
    groupId: "grp_vh_kho",
    senderId: "u_le_binh",
    sender: "Le Bình",
    type: "image",
    content: "Nhập kho hoàn tất",
    files: [      
      { name: "img7.jpg", url: img7, type: "image" },      
    ],
    ...makeTime(13, 40),
  },
  {
    id: "nh_10054",
    groupId: "grp_vh_kho",
    senderId: "u_diem_chi",
    sender: "Diễm Chi",
    isMine: true,
    type: "file",    
    fileInfo: { name: "danh_sach_hang_nhap từ An Phát.xls", url: "/files/danh_sach_hang_nhap_An_Phat.xls", type: "excel", size: "1024KB" },
    ...makeTime(14,15),
  },
  {
    id: "nh_10052",
    groupId: "grp_vh_kho",
    senderId: "u_diem_chi",
    sender: "Diễm Chi",
    isMine: true,
    type: "file",    
    fileInfo: { name: "Hóa đơn nhập An Phát.docx", url: "/files/hoa_don_nhap_An_Phat.docx", type: "word", size: "830KB" },
    ...makeTime(14,35),
  },
  {
    id: "msg_0003",
    groupId: "grp_vh_kho",
    senderId: "u_thanh_truc",
    sender: "Thanh Trúc",
    type: "text",    
    content: "Cân hàng nhập cuối ngày",
    ...makeTime(18, 45),
    taskId: "task_004",
  },
  {
    id: "nh_10053",
    groupId: "grp_vh_kho",
    senderId: "u_thanh_truc",
    sender: "Thanh Trúc",
    isMine: false,
    type: "file",    
    fileInfo: { name: "Danh sách kiểm đếm.xlsx", url: "/files/hoa_don_nhap_An_Phat.xlsx", type: "excel", size: "830KB" },
    ...makeTime(14,35),
  },
  {
    id: "msg_0030",
    groupId: "grp_vh_kho",
    senderId: "u_admin",
    sender: "Admin",
    type: "text",
    content: "Kế hoạch nhập hàng tháng 12 cần được phê duyệt trước ngày 15.",
    ...makeTime(8, 0),
    taskId: "task_leader_001",
  },
  {
    id: "msg_0031",
    groupId: "grp_vh_kho",
    senderId: "u_admin",
    sender: "Admin",
    type: "text",
    content: "Cần đánh giá KPI team tháng 11 và gửi báo cáo lên giám đốc.",
    ...makeTime(9, 0),
    taskId: "task_leader_002",
  },
  {
    id: "msg_0032",
    groupId: "grp_vh_kho",
    senderId: "u_admin",
    sender: "Admin",
    type: "text",
    content: "Họp với NCC CP về vấn đề chất lượng hàng hóa buổi sáng nay.",
    ...makeTime(7, 30),
    taskId: "task_leader_003",
  },
  {
    id: "msg_0033",
    groupId: "grp_vh_kho",
    senderId: "u_admin",
    sender: "Admin",
    type: "text",
    content: "Kiểm tra báo cáo tồn kho tuần và xác nhận với kế toán.",
    ...makeTime(10, 15),
    taskId: "task_leader_004",
  },
  {
    id: "msg_0034",
    groupId: "grp_vh_kho",
    senderId: "u_admin",
    sender: "Admin",
    type: "text",
    content: "Đề xuất mua 3 xe nâng mới cần được phê duyệt.",
    ...makeTime(8, 45),
    taskId: "task_leader_005",
  },
  {
    id: "msg_0035",
    groupId: "grp_vh_kho",
    senderId: "u_admin",
    sender: "Admin",
    type: "text",
    content: "Đào tạo quy trình nhập kho điện tử cho toàn team.",
    ...makeTime(14, 0),
    taskId: "task_leader_006",
  },
  {
    id: "msg_0036",
    groupId: "grp_vh_kho",
    senderId: "u_sales",
    sender: "Phòng Bán Hàng",
    type: "text",
    content: "Đơn POS-9876 bị thiếu hàng, cần xử lý khiếu nại gấp.",
    ...makeTime(11, 20),
    taskId: "task_leader_007",
  },
  {
    id: "msg_0037",
    groupId: "grp_vh_kho",
    senderId: "u_admin",
    sender: "Admin",
    type: "text",
    content: "Lập kế hoạch kiểm kê tồn kho cuối tháng 11.",
    ...makeTime(9, 30),
    taskId: "task_leader_008",
  },
  {
    id: "msg_0038",
    groupId: "grp_vh_kho",
    senderId: "u_admin",
    sender: "Admin",
    type: "text",
    content: "Đánh giá hiệu suất các nhà cung cấp trong tháng 11.",
    ...makeTime(15, 0),
    taskId: "task_leader_009",
  },
  {
    id: "msg_0039",
    groupId: "grp_vh_kho",
    senderId: "u_admin",
    sender: "Admin",
    type: "text",
    content: "Họp ban lãnh đạo về chiến lược kinh doanh quý 4.",
    ...makeTime(8, 0),
    taskId: "task_leader_010",
  },
];

export const doiTraMessages: Message[] = [
  {
    id: "sys_dt_0001",
    groupId: "grp_vh_kho",
    senderId: "system",
    sender: "System",
    type: "system",
    content: "Wednesday, Nov 12",
    ...makeTime(8, 45),
  },
  {
    id: "dt_2001",
    groupId: "grp_vh_kho",
    senderId: "u_le_binh",
    sender: "Lệ Bình",
    type: "text",
    content: "Em tổng hợp danh sách đổi trả tuần này.",
    ...makeTime(8, 50),
  },
  {
    id: "dt_2002",
    groupId: "grp_vh_kho",
    senderId: "u_le_binh",
    sender: "Lệ Bình",
    type: "file",
    content: "Danh sách đổi trả tuần 45",
    fileInfo: { name: "doi_tra_tuan45.xlsx", url: "/files/doi_tra_tuan45.xlsx", type: "excel", size: "128KB" },
    ...makeTime(8, 51),
  },
  {
    id: "dt_2003",
    groupId: "grp_vh_kho",
    senderId: "u_thanh_truc",
    sender: "Thanh Trúc",
    type: "text",
    content: "Đã xem, gửi NCC trước 10h nhé.",
    ...makeTime(8, 55),
    replyTo: {
      id: "dt_2002",
      type: "file",
      sender: "Lệ Bình",
      content: "Danh sách đổi trả tuần 45",
      fileInfo: { name: "doi_tra_tuan45.xlsx", url: "/files/doi_tra_tuan45.xlsx", type: "excel" },
    },
  },
  {
    id: "dt_2004",
    groupId: "grp_vh_kho",
    senderId: "u_diem_chi",
    sender: "Diễm Chi",
    isMine: true,
    type: "image",
    content: "Hàng lỗi cần đổi trả",
    files: [
      { name: "img1.jpg", url: img1, type: "image" },      
    ],
    ...makeTime(11, 5),
  },
  {
    id: "dt_20042",
    groupId: "grp_vh_kho",
    senderId: "u_thanh_truc",
    sender: "Thanh Trúc",
    type: "image",
    content: "Hàng lỗi cần đổi trả",
    files: [      
      { name: "img3.jpg", url: img3, type: "image" },      
    ],
    ...makeTime(11, 5),
  },
  {
    id: "dt_20043",
    groupId: "grp_vh_kho",
    senderId: "u_thanh_truc",
    sender: "Thanh Trúc",
    type: "text",
    content: "Phê duyệt kế hoạch nhập hàng tháng 12",
    ...makeTime(13, 8),
  },
  {
    id: "dt_20044",
    groupId: "grp_vh_kho",
    senderId: "u_le_binh",
    sender: "Le Bình",
    type: "image",
    content: "Chuẩn bị nhận hàng mới",
    files: [      
      { name: "img5.jpg", url: img5, type: "image" },      
    ],
    ...makeTime(13, 40),
  },
  {
    id: "dt_20045",
    groupId: "grp_vh_kho",
    senderId: "u_le_binh",
    sender: "Lệ Bình",
    type: "text",
    content: "Bắt đầu nhận hàng mới từ NCC.",
    ...makeTime(13, 8),
  },
];

export const lichBocHangMessages: Message[] = [
  {
    id: "sys_lb_0001",
    groupId: "grp_vh_taixe",
    senderId: "system",
    sender: "System",
    type: "system",
    content: "Wednesday, Nov 12",
    ...makeTime(14, 0),
  },
  {
    id: "lb_3001",
    groupId: "grp_vh_taixe",
    senderId: "u_huyen",
    sender: "Huyền",
    type: "text",
    content: "Lịch bốc hàng mai: Long An – Bình Dương (3 xe).",
    ...makeTime(14, 15),
  },
  {
    id: "lb_3002",
    groupId: "grp_vh_taixe",
    senderId: "u_ngoc_vang",
    sender: "Ngọc Vàng",
    type: "image",
    content: "Bảng điều xe ngày mai",
    files: [{ name: "img3.jpg", url: img3, type: "image" }],
    ...makeTime(14, 18),
  },
  {
    id: "lb_30031",
    groupId: "grp_vh_taixe",
    senderId: "u_thanh_truc",
    sender: "Thanh Trúc",
    type: "text",
    content: "Em tổng hợp danh sách đổi trả tuần này.",
    ...makeTime(8, 50),
  },
];

export const donBocHangMessages: Message[] = [
  {
    id: "sys_db_0001",
    groupId: "grp_vh_taixe",
    senderId: "system",
    sender: "System",
    type: "system",
    content: "Wednesday, Nov 12",
    ...makeTime(9, 55),
  },
  {
    id: "db_4001",
    groupId: "grp_vh_taixe",
    senderId: "u_huyen",
    sender: "Huyền",
    type: "text",
    content: "Đơn bốc #DH20251112-1: Xuất 150 thùng mì về Bình Dương.",
    ...makeTime(10, 0),
  },
  {
    id: "db_4002",
    groupId: "grp_vh_taixe",
    senderId: "u_ngoc_vang",
    sender: "Ngọc Vàng",
    type: "file",
    content: "Đơn bốc hàng DH20251112-1",
    fileInfo: { name: "DH20251112-1.pdf", url: "/files/DH20251112-1.pdf", type: "pdf" },
    ...makeTime(10, 2),
  },
  {
    id: "db_4003",
    groupId: "grp_vh_taixe",
    senderId: "u_thanh_truc",
    sender: "Thanh Trúc",
    type: "text",
    content: "Đơn này giao cho tài xế Hậu phụ trách.",
    ...makeTime(10, 5),
    replyTo: {
      id: "db_4001",
      type: "text",
      sender: "Huyền",
      content: "Đơn bốc #DH20251112-1: Xuất 150 thùng mì về Bình Dương.",
    },
  },
  {
    id: "db_4004",
    groupId: "grp_vh_taixe",
    senderId: "u_huyen",
    sender: "Huyền",
    type: "image",
    content: "Ảnh đóng hàng DH20251112-1",
    files: [
      { name: "img1.jpg", url: img1, type: "image" },
      { name: "img2.jpg", url: img2, type: "image" },
      { name: "img4.jpg", url: img4, type: "image" },
    ],
    ...makeTime(10, 20),
  },
];

export const mockMessagesByWorkType = {
  nhanHang: nhanHangMessages,
  doiTra: doiTraMessages,
  lichBocHang: lichBocHangMessages,
  donBocHang: donBocHangMessages,
};
